...
describe ReportsController do
  ...
  describe '#vendor_spend_data' do
    it "must return correct orders for a given day" do
      ...
      get :vendor_spend_data, from: (Time.now - 7.days).strftime("%d-%m-%Y"), to: (Time.now).strftime("%d-%m-%Y")
  ...
  describe "#category_spend_data" do
    ...
    describe "complex stuff" do
      ...
      it "should return correct data for current month" do
        get :category_spend_data, from: Time.now.beginning_of_month, to: end_of_month
        body = JSON.parse(response.body)
        body.map{|cs| cs['spend']}.must_include "200.00", "400.00"
      end

      it "should return correct data for past month" do
        get :category_spend_data, from: (Time.now - 1.month).beginning_of_month, to: (Time.now - 1.month).end_of_month
        body = JSON.parse(response.body)
  ...
  describe "#items_spend_data" do
    ...
    it "returns all data if there's no date range" do
      get :items_spend_data, from: Date.today - 7.days, to: Date.today
      body = JSON.parse(response.body)
    ...
    it "returns scoped data for a given day" do
      6.times do |i|
        get :items_spend_data, from: Date.today - (i+1).days, to: Date.today
        body = JSON.parse(response.body)
  ...
  describe "#item_orders_chart_data" do
    it "returns correct data" do
      ...
      get :item_orders_chart_data, id: item.id, from: '01-07-2014', to: '30-09-2014'
      ...
      describe role_name do
        ...
        describe "#category_spend" do
          ...
          it "should have access to #category_spend_data" do
            get :category_spend_data, from: (Time.now - 7.days).strftime("%d-%m-%Y"), to: (Time.now).strftime("%d-%m-%Y")
            assert_response :success
          end
        end
        ...
        describe "#item_price_variance" do
          ...
          it "should have access to #item_price_variance_data" do
            get :item_price_variance_data, from: (Time.now - 7.days).strftime("%d-%m-%Y"), to: (Time.now).strftime("%d-%m-%Y")
            assert_response :success
          end
        end
        ...
        describe "#items_consumption" do
          ...
          it "should have access to #items_consumption_data" do
            get :items_consumption_data, from: (Time.now - 7.days).strftime("%d-%m-%Y"), to: (Time.now).strftime("%d-%m-%Y")
            assert_response :success
          end
        ...
        describe "#items_spend" do
          ...
          it "should have access to #items_spend_data" do
            get :items_spend_data, from: (Time.now - 7.days).strftime("%d-%m-%Y"), to: (Time.now).strftime("%d-%m-%Y")
            assert_response :success
          end
        end
