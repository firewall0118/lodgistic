...
describe 'Purchase Orders Integration' do
  ...
  it 'shows all items in order detail page' do
    ...
    find('h3.semibold.text-success', text: '$200.00')
  end
  ...
  it 'should show VPT action', js: true do
    ...
    visit purchase_order_path(@po)
    find('a.order-action-btn', text: 'Send to Vendor').trigger(:click)
    find("a", text: 'YES I\'m sure!').trigger(:click)
    sleep 4
    response_headers['Content-Type'].must_include 'application/xml'

    # check xml content with service after we integrate it
  end
