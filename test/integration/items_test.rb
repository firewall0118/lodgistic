...
describe 'Items Integration' do
  ...
  describe 'open requests/orders count' do
    ...
    it 'should display badge on item listing if at least one PR with this item still not closed' do
      i = create(:item)
      create(:purchase_request, item_requests: [ create(:item_request, item: i) ])
      visit items_path
      find('table tr td .label-danger')
    end

    it 'should display badge on item listing if at least one PO with this item still not closed' do
      i = create(:item)
      create(:purchase_order, item_orders: [ create(:item_order, item: i) ])
      visit items_path
      find('table tr td .label-danger')
    end
    ...
    it 'should display widget when clicking on badge', js: true do
      ...
      pr1.create_orders_on_approval!(user, Property.find(Property.current_id))
      # ==> END
