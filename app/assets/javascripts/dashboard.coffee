showCategorySpendChart = ->
  ...
  $.ajax(
    dataType: 'json'
    type: 'GET'
    url: Routes.category_spend_data_reports_path( from: dateRange.from4rails(), to: dateRange.to4rails() )
    success: (data) ->
      totalSpend = 0
...
showBudgetAndSpendChart = ->
  ...
  $('#budget-vs-spend-time-selector ul a').on 'click', ->
    ...
    $(@).parents('#budget-vs-spend-time-selector').find('.status').text( Date.parse('today').toString({ month: "yyyy & MMM", year: "yyyy" }[$(@).data('range')] ))
